...
const geoip_lite_1 = __importDefault(require("geoip-lite")); // You'll need to install this package
class UserDeviceController {
    // Get all devices for current user
    async getMyDevices(req, res) {
        try {
            const userId = req.user?.id;
            const { page = 1, limit = 20, activeOnly = 'true', trustedOnly = 'false' } = req.query;
            ...
            const { rows: devices, count: total } = await UserDevice_model_1.default.findAndCountAll({
                where,
    ...
    async getDeviceById(req, res) {
        try {
            ...
            const device = await UserDevice_model_1.default.findByPk(id, {
                include: [
                    {
    ...
    async registerDevice(req, res) {
        ...
        try {
            const userId = req.user?.id;
            const { deviceId, deviceName, deviceType, userAgent, fingerprint } = req.body;
            // Get IP address from request
            const ipAddress = this.getClientIP(req);
            // Get location from IP
            const location = await this.getLocationFromIP(ipAddress);
            // Check if device already exists for this user
            const existingDevice = await UserDevice_model_1.default.findOne({
                where: {
                    userId,
                    deviceId,
                    isActive: true
            ...
            if (existingDevice) {
                ...
                // Create new device
                device = await UserDevice_model_1.default.create({
                    userId,
                    deviceId,
                    deviceName,
                    deviceType,
    ...
    async updateDevice(req, res) {
        ...
        try {
            ...
            const userId = req.user?.id;
            const userRole = req.user?.role;
            const { deviceName, isTrusted, isActive } = req.body;
            // Find the device
            const device = await UserDevice_model_1.default.findByPk(id);
            if (!device) {
                return res.status(404).json({ error: 'Device not found' });
    ...
    async revokeOtherDevices(req, res) {
        try {
            const userId = req.user?.id;
            ...
            const result = await UserDevice_model_1.default.update({ isActive: false }, {
                where: {
                    userId,
                    deviceId: { [sequelize_1.Op.ne]: currentDeviceId },
                    isActive: true
                }
            });
            // Log the action
    ...
    async getDeviceStatistics(req, res) {
        try {
            ...
            const statistics = await UserDevice_model_1.default.findAll({
                where,
                attributes: [
                    'deviceType',
            ...
            const activeDevices = await UserDevice_model_1.default.count({
                where: { ...where, isActive: true }
            });
            const trustedDevices = await UserDevice_model_1.default.count({
                where: { ...where, isTrusted: true, isActive: true }
            });
            // Get device activity over time
            const activityData = await UserDevice_model_1.default.findAll({
                where,
                attributes: [
                    [(0, sequelize_1.fn)('DATE', (0, sequelize_1.col)('lastActivityAt')), 'date'],
                    [(0, sequelize_1.fn)('COUNT', (0, sequelize_1.col)('id')), 'activeDevices']
                ],
                group: [(0, sequelize_1.fn)('DATE', (0, sequelize_1.col)('lastActivityAt'))],
                order: [[(0, sequelize_1.fn)('DATE', (0, sequelize_1.col)('lastActivityAt')), 'DESC']],
                limit: 30
            });
            res.json({
                success: true,
                data: {
                    totalDevices: statistics.reduce((total, stat) => total + parseInt(stat.get('count')), 0),
                    activeDevices,
                    trustedDevices,
                    byDeviceType: statistics.map((stat) => ({
                        deviceType: stat.get('deviceType'),
                        count: stat.get('count')
                    })),
                    activityOverTime: activityData.map((stat) => ({
                        date: stat.get('date'),
    ...
    async updateDeviceActivity(req, res) {
        try {
            const { deviceId } = req.params;
            const userId = req.user?.id;
            const device = await UserDevice_model_1.default.findOne({
    ...
    async getSuspiciousDevices(req, res) {
        try {
            ...
            const where = {
                isTrusted: false,
                lastLoginAt: {
                    [sequelize_1.Op.gte]: startDate ? new Date(startDate) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) // Last 7 days
                }
            };
            if (endDate) {
                where.lastLoginAt = {
...
exports.default = new UserDeviceController();
